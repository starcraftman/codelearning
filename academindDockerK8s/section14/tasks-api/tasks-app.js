...
const SITE = `http://${process.env.AUTH_ADDRESS}`
...
const extractAndVerifyToken = async (headers) => {
  ...
  try {
    const response = await axios.get(`${SITE}/verify-token/${token}`);
    return response.data.uid;
  } catch (error) {
    throw new Error(`Failed to get: ${SITE}/verify-token/${token}`)
  }
};
...
app.post('/tasks', async (req, res) => {
  try {
    ...
    fs.appendFile(filePath, jsonTask + 'TASK_SPLIT', (err) => {
      ...
      return res.status(401).json({ message: 'Could not verify token.', error: err.toString()});
  }
});
